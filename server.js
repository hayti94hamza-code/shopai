...
// ===== USE ENVIRONMENT VARIABLES DIRECTLY (NO DOTENV) =====
      ...
      async function loadReviews() {
        ...
        try {
          ...
          if (!data.success || data.reviews.length === 0) {
            container.innerHTML = \`<div class="nefhara-review-widget"><div style="text-align:center;padding:2rem;color:#6b7280;font-size:1.2rem;">✨ No reviews yet. Generate reviews from the admin panel.</div></div>\`;
            return;
          }
          ...
          container.innerHTML = \`<div class="nefhara-review-widget"><div style="text-align:center;padding:2rem;color:#ef4444;font-size:1.2rem;">❌ Error loading reviews. Make sure the server is running.</div></div>\`;
        }
      }
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'OK', reviews: allReviews ? allReviews.length : 0, timestamp: new Date().toISOString() });
});
...
async function generateBulkReviews(count = 500, country = 'Morocco') {
  ...
  if (!shopify) { console.error('❌ Shopify not configured.'); return []; }
  let products = [];
  try {
    const shopifyProducts = await shopify.product.list({ limit: 250, fields: 'id,title,images' });
    products = shopifyProducts.map(p => ({ id: p.id, title: p.title, image: p.images && p.images.length > 0 ? p.images[0].src : null }));
    console.log(`🛒 Found ${products.length} products`);
  } catch (error) { console.error('❌ Error fetching products:', error.message); return []; }
  const reviews = [];
  for (let i = 0; i < count; i++) {
    const randomProduct = products[Math.floor(Math.random() * products.length)];
    const review = generateMockReview(randomProduct.id, randomProduct.title, randomProduct.image, country, i);
    reviews.push(review);
    if ((i + 1) % 50 === 0 || i === count - 1) { console.log(`   ✅ Generated ${i + 1}/${count}`); }
  }
  return reviews;
}
...
app.post('/api/generate-bulk', async (req, res) => {
  try {
    ...
    res.json({ success: true, totalReviews: allReviews.length, message: `Generated ${allReviews.length} reviews` });
  } catch (error) {
...
app.get('/api/reviews/:productId', (req, res) => {
  try {
    ...
    res.json({ success: true, displayed: randomReviews.length, totalAvailable: allReviews.length, reviews: randomReviews });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
